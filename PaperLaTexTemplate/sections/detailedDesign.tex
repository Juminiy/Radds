\section{Radds存储系统详细设计与实现}
	...
	本章对存储系统的各层进行详细设计与实现，针对各层内部的子系统，各层之间的接口进行详细定义。
	  ...
	  存储系统在写内存之前会将本次写操作的内容写入日志文件中。图4.1是日志系统的架构图。
   ...
   日志的内容为写入的batch编码后的信息，图4.3为日志文件的具体格式。
   \begin{figure}[H]
	   \centering
   ...
   \begin{figure}[H]
	   \centering
	   \includegraphics[width=0.40\textwidth]{pdf/journal_read.pdf}
	   \caption{日志文件读流程图}
	   \label{journal_read}
   \end{figure}
   ...
   \end{enumerate}
		...
		所有数据项的key是经过特殊编码的，这种格式称为internalKey，图4.8是内部键的具体编码格式。
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.53\textwidth]{pdf/write_merge.pdf}
			\caption{写合并的流程}
			\label{write_merge}
		\end{figure}
...
func (p *DB) randHeight() (h int) {
	...
	\end{enumerate}
		...
		\begin{enumerate}
			...
			代码清单4.2是数据的具体组织形式。
		\begin{lstlisting}[caption=storageDB , label=code_radds_storage_db]
type DB struct {
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.40\textwidth]{pdf/indexblock_format.pdf}
...
另外尾部还会存储一个magic word。图4.19是sstable底部格式图。
\begin{figure}[H]
	\centering
...
图4.20为在一个sstable中查找某个数据项的流程图。
\begin{figure}[H]
	\centering
	\includegraphics[width=0.38\textwidth]{pdf/sstable_read_procedure.pdf}
...
若在第二个data block中找到了目标数据，则返回结果；若未查找成功，则返回Not Found错误信息。在存储系统中，使用cache来缓存两类数据：
sstable文件句柄及其元数据；
data block中的数据；
...
若过滤器表示不存在，则该key一定不存在，由此加快了查找的效率。
		...
		\begin{enumerate}
			...
			bloom过滤器底层是一个位数组，初始时每一位都是0，图4.28是布隆过滤器示意图1。
			\begin{figure}[H]
				\centering
...
\begin{lstlisting}[caption=bloomFilter , label=code_radds_storage_typedef_bloomfilter]
...
\end{lstlisting}
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				图4.32是sstable的结构主归并图。
				\begin{figure}[H]
					\centering
					\includegraphics[width=0.65\textwidth]{pdf/major_compaction.pdf}
					\caption{sstable 结构主归并}
					\label{sstable_major_compaction}
				\end{figure}
				
				0层中的三个sstable文件，加上1层中的的sstable文件，四个文件进行了合并，输出成两个按序组织的新的1层sstable文件进行替换。
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.68\textwidth]{pdf/version_recover.pdf}
	\caption{版本复原示意图}
